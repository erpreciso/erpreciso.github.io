...
knitr::opts_chunk$set(echo = TRUE, fig.path = "../images/cpj/", cache = FALSE)
```
...
```{r proportions-coverage-iraq, echo = TRUE}
d %>%
    filter(Country_killed == "Iraq",
           !is.na(Year)) %>%
    select(Year, War) %>%
...
```{r iraq-war-freq-table, echo = TRUE}
d %>%
    filter(Country_killed == "Iraq") %>%
    select(Year, War) %>%
    group_by(Year, War) %>%
...
```{r murderers, echo = TRUE, cache=TRUE}
...
```{r murder-by-countries, echo = TRUE}
murder_countries <- d %>%
...
```{r impunity-by-country, echo = TRUE}
d_murder <- d %>%
    tbl_df() %>%
...
From this data there is `r d_murder %>% group_by(Impunity_for_murder) %>% summarise(n = n()) %>% mutate(p = n / sum(n)) %>% filter(Impunity_for_murder == "No") %>% mutate(p = scales::percent(p)) %>% select(p)` possibility to have justice after a murder case.
...
```{r impunity-iraq, echo = TRUE}
plot_impunity(1)
```

### Philippines

```{r impunity-philippines, echo = TRUE}
plot_impunity(2)
```

### Algeria

```{r impunity-algeria, echo = TRUE}
plot_impunity(3)
```
...
```{r impunity-by-perpetrator, echo = TRUE}
